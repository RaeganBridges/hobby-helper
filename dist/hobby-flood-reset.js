...
/**
 * After week 4 is marked played: fade hobby flood art in and out (same image as title flood),
 * then run onDone() (e.g. open the week-4 video).
 */
(function () {
  var FADE_MS = 640;
  var HOLD_MS = 520;

  window.runHobbyCompletionFlood = function (onDone) {
    var CFG = window.HOBBY_FLOOD_RESET;
    ...
    var imgSrc = CFG.completionFloodSrc || CFG.secondSrc;
    if (!imgSrc) {
      onDone();
      return;
    }
    if (
      document.querySelector('.hh-flood-overlay-root') ||
      document.querySelector('.hh-flood-completion-root')
    ) {
      onDone();
      return;
    }
    ...
    document.documentElement.style.setProperty('--hh-completion-fade-ms', FADE_MS + 'ms');

    var root = document.createElement('div');
    root.className = 'hh-flood-completion-root';
    root.setAttribute('aria-hidden', 'true');
    root.innerHTML =
      '<div class="hh-flood-shell">' +
      '<div class="hh-flood-stage">' +
      '<img class="hh-flood-img" alt="" decoding="async" src="' +
      imgSrc +
      '" />' +
      '</div></div>';
    ...
    requestAnimationFrame(function () {
      requestAnimationFrame(function () {
        root.classList.add('hh-flood-completion-visible');
      });
    });

    timers.push(
      window.setTimeout(function () {
        root.classList.remove('hh-flood-completion-visible');
